...
public:
    ...
    void AppendTags(std::pair<std::string, std::string>&& tag) { tags_.emplace_back(std::move(tag)); }
...
public:
    ...
    explicit __attribute__((visibility("default")))
    ApplicationBatchEvent(const std::string& app_id, std::vector<std::pair<std::string, std::string>>&& tags)
        : app_id_(app_id), tags_(tags) {}
    explicit __attribute__((visibility("default")))
    ApplicationBatchEvent(const std::string& app_id,
                          std::vector<std::pair<std::string, std::string>>&& tags,
                          std::vector<std::unique_ptr<SingleEvent>>&& events)
...
// observe metrics
using NamiHandleBatchMeasureFunc
    = std::function<void(std::vector<std::unique_ptr<ApplicationBatchMeasure>>& measures, uint64_t timestamp)>;
// observe spans
...
class PodMeta {
public:
    PodMeta(const std::string& app_id,
...
using AsyncK8sMetadataCallback
    = std::function<std::future<bool>(std::vector<std::string>&, std::vector<std::unique_ptr<PodMeta>>&)>;
...
struct NetworkObserveConfig {
    ...
    bool operator==(const NetworkObserveConfig& other) const {
        return enable_libbpf_debug_ == other.enable_libbpf_debug_ && enable_so_ == other.enable_so_
            && btf_ == other.btf_ && btf_size == other.btf_size && so_ == other.so_ && so_size_ == other.so_size_
            && uprobe_offset_ == other.uprobe_offset_ && upca_offset_ == other.upca_offset_
            && upps_offset_ == other.upps_offset_ && upcr_offset_ == other.upcr_offset_;
    }
};
...
struct ProcessConfig {
    ...
    bool operator==(const ProcessConfig& other) const {
        return enable_libbpf_debug_ == other.enable_libbpf_debug_ && options_ == other.options_;
    }
};
...
struct eBPFConfig {
    ...
    bool operator==(const eBPFConfig& other) const {
        return plugin_type_ == other.plugin_type_ && type == other.type && host_name_ == other.host_name_
            && host_ip_ == other.host_ip_ && host_path_prefix_ == other.host_path_prefix_ && config_ == other.config_;
    }
};
